...
TaggedUnion sigmoid_dyn(TaggedUnion x) {
    TaggedUnion ans{x.type}; // 初始化返回结果，并拷贝类型
    // TODO: 根据 type 调用 sigmoid (Done)
    switch (x.type) {
        case DataType::Float:
            // 调用 float 版本的 sigmoid
            ans.f = sigmoid(x.f); // 模板参数 T 会被自动推导为 float
            break;
        case DataType::Double:
            // 调用 double 版本的 sigmoid
            ans.d = sigmoid(x.d); // 模板参数 T 会被自动推导为 double
            break;
        // 可以添加一个 default 来处理未知的类型
    }
    return ans;
}
